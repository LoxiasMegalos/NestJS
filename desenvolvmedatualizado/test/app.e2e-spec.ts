...
describe('Testando requisições da API utilizada no app DesenvolvMED', () => {
  ...
  it('23 - Deverá listar todos os comentarios de um post especifico', async() => {

    return request(app.getHttpServer())
    .get(`/postagem/comentarios/${postagemId}`)
    .expect(200)
    .then(response => {
      expect("Que post maravilhoso e sincero!").toEqual(response.body[0].conteudo)
      expect("Adorei o post doutor, muito informativo!").toEqual(response.body[1].conteudo)
    });
  })

  it('24 - Deverá deletar um post', async () => {
  ...
  it('25 - Deverá deletar um tema', async () => {
    return request(app.getHttpServer())
      .delete('/tema/' + temaId)
      .expect(204)
  })

  it('26 - Deverá deletar um médico', async () => {
    return request(app.getHttpServer())
      .delete(`/cadastro/${medicoCadastroGeralId}`)
      .expect(204)
  })

  it('27 - Deverá deletar um paciente', async () => {
    return request(app.getHttpServer())
